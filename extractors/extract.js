 ...
 *
 * @param {string} filePath - a path to the input file
 ...
 *
 * @param {string} connectionString - connection string for the Postgres database
 * @param {string} tableName - name of the desired table
...
extract.fromPostgres = (connectionString, tableName) => {
  ...
  return Observable.create((observer) => {
    ...
    pgClient.connect(function (err, client) {
      ...
      stream.on('end', () => {
        // Capturing end time for performance testing
        const end = now();
        // Closing database connection
        pgClient.end();
        // Completing observer
